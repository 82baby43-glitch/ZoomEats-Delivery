...
import { ChefHat, Truck, TrendingUp, Bell } from "lucide-react";

export default function RestaurantLiveMapDashboard() {
  const [theme, setTheme] = useState("dark");
  const fetchRestaurant = useCallback(() => api.get("/logistics/restaurant"), []);
  const { data, loading, error, reload } = useLogisticsPoll(fetchRestaurant, "restaurant");
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(data?.active_orders || []).map((o) => (
              <div key={o.order_id} className="card p-4" data-testid={`restaurant-order-${o.order_id}`}>
                <div className="flex justify-between gap-2">
                  <div>
                    <div className="font-bold">{o.customer_name}</div>
                    <div className="text-sm" style={{ color: "var(--muted)" }}>{o.driver_name} · ${formatMoney(o.order_value)}</div>
                  </div>
                  <span className="badge capitalize">{o.live_status}</span>
                </div>
                <div className="flex flex-wrap gap-2 mt-3 text-xs">
                  {o.eta_pickup_min != null && <span className="badge">Pickup {o.eta_pickup_min}m</span>}
