...
export async function getFinancialAnalytics(
  ...
  return {
    ...
    trends: {
      ...
      gmv: bucketByDay(snaps.length ? snaps : orders, (r) => Number(r.subtotal ?? 0)),
      ...
      promotion_costs: bucketByDay(
        platform.length ? platform : snaps,
        (r) => Number(r.promotion_cost ?? r.discount_amount ?? 0)
      ),
