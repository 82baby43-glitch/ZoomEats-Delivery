...
import { handleVendorOrderReady } from "./delivery/handler.ts";
import { recordDeliveryEvent } from "./delivery/workflow.ts";
import { buildRestaurantLogisticsView } from "./logistics/engine.ts";
import { getWeeklyPayoutSummary } from "./restaurantCommission/engine.ts";
import { syncRestaurantLaunchState } from "./restaurant/readiness.ts";
import { loadMerchantComplianceProfile, syncMerchantComplianceProfile } from "./merchant/complianceProfile.ts";
import { DISPENSARY_SLUG } from "./merchant/categoryConfig.ts";

function isPaymentConfirmed(order: { payment_status?: unknown } | null | undefined): boolean {
...
export async function handleVendorDashboardRequest(
  ...
  if (path === "/vendor/dashboard" && method === "GET") {
    ...
    return {
      ...
      stats,
      pending_payout: payout?.pending_amount ?? payout?.estimated_payout ?? 0,
