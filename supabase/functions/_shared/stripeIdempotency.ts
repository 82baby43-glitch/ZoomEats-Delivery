...
export async function alreadyProcessedEvent(db: SupabaseClient, eventId: string): Promise<boolean> {
  if (memoryAlreadyProcessed(`evt:${eventId}`)) return true;
  const exists = await dbEventExists(db, eventId);
  if (exists) memoryMarkProcessed(`evt:${eventId}`);
  return exists;
}
