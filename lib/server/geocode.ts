/** Server-side address geocoding — Google Maps if configured, else OpenStreetMap Nominatim. */
...
function getGeocodeProvider(): GeocodeProviderMode {
  const raw = (process.env.GEOCODE_PROVIDER || "auto").toLowerCase();
...
function getNominatimBaseUrl(): string {
  return (process.env.NOMINATIM_API_URL || "https://nominatim.openstreetmap.org").replace(/\/$/, "");
}

function defaultRegion(): string {
  return process.env.DEFAULT_GEOCODE_REGION || "Columbia, MO, USA";
}
