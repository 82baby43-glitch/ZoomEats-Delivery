...
import { getProfitProtectionSummary } from "../pricing/profitProtection";
import { calculatePricingQuote } from "../pricing/engine";
import { getPromotionBudgetStatus } from "../pricing/promotionBudget";
import { PRICING_RULES_DASHBOARD } from "../pricing/ruleFields";
...
export async function handlePricingAdminRequest(
  ...
  if (path === "/admin/pricing/summary" && method === "GET") {
    const [{ data: snapshots }, { data: rules }, profitSummary, promotionBudget] = await Promise.all([
