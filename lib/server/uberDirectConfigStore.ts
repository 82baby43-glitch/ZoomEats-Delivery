...
import { decryptUberSecret, encryptUberSecret } from "./uberDirectCrypto";
import { getUberDirectConfigFromEnv, type UberDirectConfig, type UberDirectConfigRow } from "./uberDirectEnv";
...
async function syncUberDirectConfigFromEnvIfNeeded(
  db: SupabaseClient,
  ...
  const fromEnv = getUberDirectConfigFromEnv(env);
  if (!fromEnv) return;
...
export async function resolveUberDirectConfig(
  db?: SupabaseClient | null,
  env: NodeJS.ProcessEnv = process.env
): Promise<UberDirectConfig | null> {
  if (db) {
    await syncUberDirectConfigFromEnvIfNeeded(db, env);
    const row = await loadUberDirectConfigRow(db);
