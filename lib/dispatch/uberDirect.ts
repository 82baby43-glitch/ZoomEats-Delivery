/**
 * Uber Direct API client — create quotes and deliveries for ZoomEats orders.
...
function parseUsAddress(address: string): UberAddressParts {
...
export async function createUberDeliveryQuote(
  ...
  const data = await uberFetch<{ id: string; fee?: number }>(
    cfg,
