...
export default function Chatbot() {
  ...
  return (
    <>
      <button
        onClick={() => setOpen((v) => !v)}
        className="fixed bottom-6 right-6 w-14 h-14 rounded-full text-white flex items-center justify-center shadow-lg transition-transform hover:scale-105 z-50"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 w-[360px] max-w-[calc(100vw-3rem)] h-[480px] bg-white border rounded-2xl shadow-2xl flex flex-col z-50 overflow-hidden"
            style={{ borderColor: "var(--border)" }}
            data-testid="chatbot-panel"
          >
            <div className="px-4 py-3 border-b" style={{ borderColor: "var(--border)", background: "var(--accent)", color: "#fff" }}>
              <div className="font-display font-bold">Zoey · Food concierge</div>
              <div className="text-xs opacity-80">Powered by Claude · Tell me what you crave</div>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3" data-testid="chatbot-messages">
              ...
              {busy && (
                <div className="flex justify-start">
                  <div className="px-3 py-2 rounded-2xl text-sm" style={{ background: "var(--bg)" }}>
